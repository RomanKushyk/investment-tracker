...
4. **Failing test first**, committed alone. Then the change. Then `pnpm lint && pnpm typecheck && pnpm test && pnpm format:check`; `pnpm exec tsc --noEmit -p infra` from the root when `infra/` or `packages/core/` changes. UI: verify in the running browser at desktop and 360.
