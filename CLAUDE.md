...
2. Pick a `Ready` issue with no open blocker — from the lowest open version milestone, or an `observation` once the date in its title has come, whatever its milestone, and never before; a `bug` goes first. Move it to `In progress` — that column must be empty before. One issue at a time. A multi-part ask that will not fit one issue goes through the `plan-epic` skill first and lands as an epic with `Ready` sub-issues to pick from.
