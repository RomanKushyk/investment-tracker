...
- **docs/BUILD-PLAN.md is the living plan** — pinned contracts (types, repo/hooks/store APIs, token names), task checkboxes, Status table, design-HTML line map. Pick up the first non-done task there and keep it updated. Decisions log: docs/DECISIONS.md.
...
- Base branch: `dev`. Feature branches `<type>/<kebab-title>` (e.g. `feat/daily-quotes`), plain conventional commits, squash-merge back to `dev`. No remote configured yet.
