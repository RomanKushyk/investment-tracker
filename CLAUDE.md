...
- **No merge commits — ever.** Integrate by **rebase** or **squash** only, and promote `dev` into `main` by fast-forward. Enforced, not merely agreed: merge commits are disabled at the repo level and a ruleset requires linear history on both long-lived branches.
