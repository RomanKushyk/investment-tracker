...
- **The logo is mark 04** — four bars, height is value and opacity is age. It lives in `src/app/Sidebar.tsx` (`Mark`) and `public/favicon.svg`; the two duplicate the geometry deliberately and must change together. The sidebar circle no longer carries ₴/$.
