...
| `markdown-files.ts` | `markdownFiles(dir)` and `REPO`, the repo walk and root this scanner, `../docs-line-cap.test.ts` and `../decisions/records.ts` (a third consumer, for `REPO` alone) all use, and its `SKIP` list (`node_modules`, `dist`, `.git`, … `.superpowers` — gitignored scratch), which answers **"not ours to MEASURE" and nothing else** — a superset of `../scratch-dirs.ts`'s `PARITY` ("not ours to TRACK"), deliberately separate because `.claude` belongs here whole while only `worktrees/` under it is untrackable, and a tracked directory could reasonably join this list without ever belonging in a `.gitignore`. Exported so `../nested-checkouts.test.ts` can assert the one direction that holds: everything hidden from the tools is outside this walk too (D109); `repoFiles(dir, extensions)` generalises the same walk for `../claims/target-files.ts`, which needs `.sql`/`.ts`/`.tsx` too — `markdownFiles` is now a thin `repoFiles(dir, ['.md'])` wrapper over it |
