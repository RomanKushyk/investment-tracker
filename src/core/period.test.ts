...
// Every case fixes `latest` at the seed's own last snapshot date so the arithmetic
// is checkable by hand against a real portfolio rather than a round number.
...
describe('resolveWindow', () => {
  ...
  it('counts months back from the LATEST snapshot, not from today', () => {
    // Today is not a portfolio fact. The last snapshot is, and it is what every other
    // figure on these screens is measured to.
  ...
  it('clamps to the start when the option reaches further back than the data', () => {
    // The seed is under six months old, so 6m and 12m are both "since start" wearing a
    // longer name — the exact thing G-3 forbids showing silently.
  ...
  it('"all" is not clamped even though it starts at the start', () => {
    // The flag means "you asked for more than exists", not "from == start". Marking
    // `all` would put a warning on the default state.
  ...
  it('clamps the day of month rather than overflowing it', () => {
    // 31 March minus one month is February, which has no 31st. `addMonths` already
    // clamps to the last day; this asserts the window inherits that.
