...
/**
 * Every type against every predicate, in ONE `Record<TxType, …>` rather than three
 * overlapping ones. A record and not a list, because a list can be under-written and
 * a record cannot: a ninth `TxType` has no row here and the BUILD stops, before any
 * assertion below gets to compare two sets that both forgot it.
 */
...
describe('a type-keyed site answers for every type, or it does not compile', () => {
  it('a synthetic ninth member of the union has no row, and that is the guard', () => {
    ...
    // `transfer` row, so the line below is an error, and `@ts-expect-error` turns it
    // into the assertion.
    //
    // WHAT IT GUARDS, measured on `tsc --strict`: widen `ANSWERS` to `Record<string, …>`
    // and the assignment stops erring, so TS2578 fires on the directive nothing used.
    // It does NOT catch a `Partial` — a partial source is still unassignable here, the
    // directive stays used, and the build breaks at `ANSWERS[type].asset` instead. The
    // exhaustive SWITCHES this pairs with cannot be proved from a test: a missing arm
    // is an error in the file that holds it, which is what writing them against `never`
    // buys, and `pnpm typecheck` is that half.
    //
    // The member is spelt so that it cannot BECOME a `TxType`: WERE it named after a
    // type someone might really add, the assignment would stop erring the day they
    // added it, and this line would read as an unused directive rather than as the
    // row `ANSWERS` is missing.
  ...
  it('targetsAsset: exactly deposit and withdrawal name no asset', () => {
    // The predicate used to be a NEGATION of a private list, so a ninth type was
    // "targets an asset" by default and the form would have demanded one for a row
    // that names none. It is a switch over all eight now, and `targetsAsset`'s own
    // doc comment carries the two store CHECKs it pairs with.
