...
The app's pure domain layer per decision G1 (`docs/plans/NEXT-PHASE-PLAN.md`) and `docs/decisions/README.md` D8. Consolidated in next-phase Phase 1 from v1's `src/lib/{types,derive,format,colors,asset-builder,schemas}` plus `src/screens/shared/` (now dissolved). The v1 pinned contracts (`docs/archive/BUILD-PLAN.md`) keep their exact shapes — only module paths changed.
