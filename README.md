...
- **Circles stay circles** — `AssetAvatar`, `ColorDot`, the decorative blob. One round thing among rounded rectangles reads as deliberate. Nothing else is a capsule: the `Switch` track (6) and its knob (4) follow the rule too. **The logo circle left this list with D131**: the disc behind the mark is gone and the mark's own arc is the circle, so its roundness is drawn rather than a radius — there is no `rounded-full` to exempt.
...
1. Logo lockup card (`#333338`, radius 14, padding 10px 15px, `justify-content:flex-start`): the 36px **Q-arrow mark** (D131) — an open `Q` whose tail is a sand arrow — beside the wordmark "Quirenote" over "INVEST TRACKER" microlabel. The mark stands on the plate with **no disc behind it**, and 36px is load-bearing: it is the tallest item in the row, so it and not the text sets the card's 56px height. The wordmark is JetBrains Mono ExtraBold at −2%, 14px — 16px ran under the DEMO badge. Neither part carries the currency symbol; the toggle below is the only currency indicator. The DEMO badge is absolutely pinned to the card's top-right corner at the card's own 15/10 padding and scaled to .75 — out of flow, so it cannot stretch the row.
...
- [ ] Currency toggle persists across reloads; converts sidebar capital and Overview KPIs only. **It no longer converts a logo symbol** — D56 replaced the ₴/$ in the sidebar circle with the static mark, and D131 removed the circle as well, so the toggle at the bottom is the only currency indicator. A mark that does not change on toggle is correct, not a regression.
