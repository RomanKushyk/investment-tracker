...
id: D48
date: 2026-08-11
summary: "The payload split is not needed; the missing index was — **D90** opens a question against it, **D91** measures the answer: the index was right and could serve the ORDER BY backwards, but the query's own `to_char(...) AS as_of` alias hid the column and forced a table scan. Its \"`payload_gzip` is never read\" holds on index paths and fails on a table scan"
---
