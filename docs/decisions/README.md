...
| D48 | The payload split is not needed; the missing index was — **D90 leaves one question open against it**: a full capture reads ~the whole cluster, which no index-only path explains. W6 settles it | 2026-08-11 |
...
| D64 | The capture retries by firing again, not by waiting longer — **its two DPU figures are superseded by D90**, the ruling stands | 2026-08-14 |
...
| D89 | **The backup vault takes a GOVERNANCE lock whose floor is read from the live plan** — found by a routine AWS sweep that was green everywhere else while the vault read `Locked: false`, one command from nothing (the **seventh** instance of the pattern below). Governance not compliance because compliance cannot be lifted by anyone including AWS and the schema is still moving (W3/W4); a floor and no `max` because what is worth forbidding is retention being WEAKENED, and the floor is derived from the live plan because the script only ever CREATES one — a literal would silently reject every nightly job. It does NOT stop the human admin, it turns a one-command accident into a deliberate two-step act, and it also freezes existing lifecycles. The mode is the PRESENCE of `--changeable-for-days`, not an argument. **Proved by attempting a delete** — refused with `InvalidRequestException`, 15 points intact | 2026-08-25 |
| D90 | **What a week of real DPU corrected in D64, and the question it left open against D48** — the `alreadySettled` guard costs ~73 DPU/month not ~6 (its lookup reads 117 KiB, not the 2 KiB minimum the estimate assumed), and the workload uses ~1.6% of the free tier not 0.3%. **D64's ruling is untouched** — a supporting estimate was wrong, not the conclusion. Left open: a full capture reads 34.2 MiB against 34.9 MiB of total cluster storage, which no index-only path explains; **W6 (2026-09-10) settles** whether that grows with the archive, and with it whether D48's index covers the path the capture actually takes | 2026-08-25 |

## A pattern these entries kept finding

D43, D44, D48, D49, D50, D53 and D89 are seven independent instances of one defect:
...
could not tell a re-run from a re-write, in D53 a failure handler that published no datapoint at all inside the very check written to catch this, and in D89 fifteen recovery points that nothing stopped anyone deleting. When adding a check, ask what it reads
