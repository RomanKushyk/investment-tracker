...
across the range files listed in its own table there. Append-only, and a wrong decision is superseded rather
...
| [`reference/MARKET-DATA-SOURCES.md`](reference/MARKET-DATA-SOURCES.md) | The external source map: which Ukrainian market data is machine-readable, from which network, and on what licence | Read before adding any external data source. SMIDA's feed is the live one (D82) but **we do NOT fetch it — D86 closed O25 categorically**; stockmarket.gov.ua answers `200` with data stopping 2019–2021 and is unreachable from a non-Ukrainian network |
