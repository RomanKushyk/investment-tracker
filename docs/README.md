...
| [`reference/W7-API-CONTRACT.md`](reference/W7-API-CONTRACT.md) | **W7's API on paper, before it is built (A53).** All 17 `repo` methods mapped onto `GET /state` / `POST /mutations`, the four `meta` keys sorted (**two** stay per-device — `seeded` dies with D2 and `nbu:lastRate` with D136), and the endpoint inventory — including the W8 admin surface, which had a screen and no API. | **Nothing here is built.** It marked O28, O31 and O33 rather than deciding them, and **all three are now ruled** — O31 as D133, O28 as **D136** (which re-pointed §1's rows 1-3 at `/view`), O33 as **D137** (which pins `asset.delete`: a batched application cascade under non-cascading keys). The reason it declined to pick a cascade stands: an API document that quietly picked one would be the implicit resolution Plan C forbids |
