...
| [`reference/W7-API-CONTRACT.md`](reference/W7-API-CONTRACT.md) | **W7's API on paper, before it is built (A53).** All 17 `repo` methods mapped onto `GET /state` / `POST /mutations`, the four `meta` keys sorted (three stay per-device, `seeded` dies with D2), and the endpoint inventory — including the W8 admin surface, which had a screen and no API. | **Nothing here is built.** It marks O28, O33 and O31 rather than deciding them: an API document that quietly picked a cascade would be the implicit resolution Plan C forbids |
