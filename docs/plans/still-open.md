...
| D137 — deletion is batched, under non-cascading keys (action settled by **D138**) | **W7's own work, and the obligations are enumerated in W7's BODY** — [`phase-w-i-ii-iii.md`](phase-w-i-ii-iii.md), under *what the foreign-key ruling puts on this phase*. **This row deliberately does not repeat or count them**: it did twice, and both times the count went stale the moment the list grew — the same defect D138's procedural lesson catalogues. The body is what an implementer reads; this file's own header forbids implementing from it | `PLAN-WAITING.md` W7 |
