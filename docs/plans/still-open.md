...
| D137 — deletion is batched, with `RESTRICT` keys | **W7's own work**, and one thing that is not: `infra/migrations/drafts/README.md`'s third promotion rewrite rule stops being conditional — append `NOT VALID` to every generated `ADD CONSTRAINT`. The schema gains `references()` with `ON DELETE RESTRICT`; the API's `asset.delete` batches children first and the parent last | `PLAN-WAITING.md` W7 |
