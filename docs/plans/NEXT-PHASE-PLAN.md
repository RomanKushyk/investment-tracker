...
| **G2's `deleteAsset`** | Owner ruling: assets accumulate, nothing is deleted. The method stays in the codebase (it is tested and harmless) but no new surface may depend on cascade-delete semantics. **[Read 2026-09-03, owner: W7's `asset.delete` is NOT a new surface — it is this same dialog over HTTP, so the row stands unchanged and D137/D138 do not supersede it. A delete control on W14's assets tab WOULD be one, and W14 still forbids it. See [D138](../decisions/D138.md).]** |
