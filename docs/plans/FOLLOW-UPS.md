...
| 12 | `.github/workflows/deploy-frontend.yml` + `deploy-backend.yml` | Every pinned action still targets **Node.js 20**, which GitHub deprecated; the runner already forces them onto Node 24 and annotates each run. Both deploys pass today — this is a warning, not a failure — but the forcing is a grace period, not a guarantee, and it will become a hard failure when the runners drop the shim. Affected across the two files: `actions/checkout@v4` (×2), `actions/setup-node@v4` (×2), `aws-actions/configure-aws-credentials@v4` (×2), `aws-actions/setup-sam@v2` (×1). | Bump each to its current major (`actions/checkout@v5`, `actions/setup-node@v5`, and whatever `aws-actions/*` has released), then push a no-op commit to `dev` and confirm both workflows go green with no Node annotation. Backend first — it is the one that can be re-run without touching the site. |
