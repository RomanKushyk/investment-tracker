...
*scanned*): ~6,506 DPU/month at year 20 = 6.5% of the always-free allowance **[Per the Cloud target decision:
...
| **`status`** | Currently **discarded** by `parse.ts:98-102`. Flips without the price changing; the flip date is gone forever if not captured that day. Captured verbatim, never filtered (The price archive decision) |
...
what the Persistence today decision removed.
...
> **Corrected 2026-08-24, per the External sources decision.** The retirement was `stockmarket.gov.ua`'s,
...
> settled: no, categorically, per the External sources decision (O25).** Our code does not
> fetch SMIDA.
