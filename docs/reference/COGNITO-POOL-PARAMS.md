...
- **Only two tokens end a family when revoked: the live one and the sign-in's original.** After `RevokeToken` on the newest token, it and the original both answer `NotAuthorizedException` — what sign-out relies on — and revoking the original ends every branch, a fork's included. Revoking any other rotated-out token, a mid-chain one or a sibling forked inside the window, leaves the newest refreshing. So a revoke on replay ends the family only when the replayed token is the original, which the relay does not keep; any other replay ends only the session of the browser that sent it.
