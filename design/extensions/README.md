...
| `muted-legibility.dc.html` | — | **The `muted` step, re-derived (D68), from FOLLOW-UPS 15.** The defect drawn at the sizes it actually occurs — `muted` #8b8a86 measured on the rendered app at **2.88 on `panel`, 3.17 on `page`, 3.46 on `card`** across 258 elements and ten routes, none of which reaches WCAG's large-text exemption (T1); the arithmetic that picks `#696865` over the two lighter candidates, including the one that lands exactly ON the 4.5 bar (T2); `label` retired into `muted`, because two tokens that must hold the same luminance to be legible are one step drawn twice (T3); and what does NOT move — the whole dark sheet, `faint`, the asset hues (T4). **Mints no hue** — it moves one token down its own warm axis and deletes another. Light theme only: dark measures 5.56–6.53 and is untouched. |
