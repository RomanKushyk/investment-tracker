...
| `extensions/appearance-language.dc.html` | Phase 5 appearance & language: theme control (Light/Dark/System), language control (Українська/English), the complete dark palette for all 57 tokens with measured WCAG ratios, charts in dark, and Ukrainian copy in situ. Its header comment records the finding that forced D54/D55 — neither brand face carries a Cyrillic glyph. Amended 2026-08-12 to the D56 radius rule (see the note at the top of this file). |
| `extensions/muted-legibility.dc.html` | The `muted` step re-derived (D68, from FOLLOW-UPS 15): the light-theme token measured on the rendered app at 2.88 / 3.17 / 3.46 against `panel` / `page` / `card` — all below the 4.5 that 9.5–13 px body text needs — the arithmetic that lands on `#696865`, and `label` retired into `muted` because both had to hold the same luminance to be legible. Mints no hue; dark is untouched. |
| `extensions/mobile.dc.html` | Phase 6 mobile shell: collapsible sidebar (open + closed, light + dark), the header bar that carries Total capital, the record card replacing all four tables, daily quotes with the keyboard open and its sticky action bar, the four overlays at 360 px, and tap-to-pin charts — plus the minted `--color-scrim` and the finding that the dark drawer needs a `sidebar-muted` edge because the scrim cannot separate it (1.02:1). |
| `extensions/scroll-surface.dc.html` | The app's ONE scroll surface (D65): rail 12 = 2+2+4+2+2, thumb r1, rail r5, margin 8 on all four sides, gutter `2m + 12` = 28 taken from the ScrollArea ROOT's padding so the bar pushes content over instead of floating above it. Supersedes `mobile.dc.html` § S5. |
| `extensions/where-things-live.dc.html` | Phase 7 information architecture: a page is editable where it shows STORED data; the edit affordance in its batch and per-entity variants; the `Entry` nav group and the `/transactions` route drawn as TWO COLUMNS (form `flex:0 1 360px`, ledger `flex:1 1 560px`); collapsible sidebar groups. |
| `extensions/period-and-analytics.dc.html` | Phase 8: the period control in `ScreenHeader`'s action slot, `/yield` under a window, `/overview`'s time dimension, `/seasonality`'s month axis, and the home for A25's portfolio XIRR. Its findings outweigh its drawings — see `extensions/README.md`. |
| `extensions/screen-density.dc.html` | The `/` screen's density (A34): the composition capped at **944 = 560 + 24 + 360** and centred, the title block hoisted OUT of the ritual column, the permanent rail and its four blocks, and the yield card recomposed for a narrow column — which also repairs it at 360, where it ships today as an eleven-line ribbon with a word split across two lines. **Two of its findings contradict the brief that commissioned it** (F-4 the unreachable `<60 px`, F-6 the caps that must be container-scoped). Mints no token. |
