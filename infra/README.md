...
| `scripts/generate-openapi.ts` | What `pnpm openapi` runs. Writes [`../docs/reference/openapi.json`](../docs/reference/openapi.json), which is an ARTIFACT — edit the handlers or the template, never the JSON. **View it locally with `npx @redocly/cli preview-docs docs/reference/openapi.json`**, which needs no hosting and no route. **`npx @redocly/cli lint docs/reference/openapi.json`** is the second reader, and deliberately not a gate of any kind: the ones this repository runs are offline and this one downloads a package on every run. What it has caught — the missing hosts, and a public route whose posture was implied rather than stated — is held by `src/openapi.test.ts` instead, because a claim about an ungated tool is one nothing notices going stale |
