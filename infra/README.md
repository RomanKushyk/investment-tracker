...
| `src/authorize.ts` | The gate every authenticated route calls. Loads the `app_user` row by the token's `sub` or its address and answers `status` and `role` — never `cognito:groups`, which is stamped at issue time and stays stamped until the token expires. Three distinguishable refusals and none of them a 401: a `pending` caller is signed in and may not act yet. With open registration on it creates the row from the token's own claims — the ID token alone, and only where the provider asserts the address verified |
...
| `src/pool-usage.ts` | Prod's Cognito pool user count, published daily as a number. There is no CloudWatch MAU metric, so total users stands in for one — a strict UPPER bound, which is what lets the alarm at 8,000 (80% of the 10,000 Essentials bills nothing for) fire early and never late, deliberately ahead of the 85% at which AWS mails the root account. Prod only, in the user stack, with three alarms beside it for the reason the backup check gives. Its one grant is `cognito-idp:DescribeUserPool` on that pool — pool CONFIGURATION, so no attribute, list or user datum becomes reachable. A count it could not read THROWS: zero is the healthy side of a `GreaterThan`, the inversion `backup-age.ts` refuses in the other direction |
...
- **The pool's user count is the free-tier watch, and it is a bound rather than a
  measurement.** There is no CloudWatch metric for monthly actives, so prod's pool
  publishes `EstimatedNumberOfUsers` daily and the alarm sits at 8,000 — 80% of the
  10,000 Essentials bills nothing for, deliberately ahead of the 85% at which AWS
  mails the root account, because a guard that fires with the bill is not a guard.
  Every monthly active is a user the pool still holds, and users who went quiet are
  counted too, so the number bounds prod's actives from ABOVE. It bounds the ACCOUNT's
  only in part: the allowance is per account and dev's pool spends it too, so this is
  prod's share rather than the whole. The one grant is
  `cognito-idp:DescribeUserPool` on that pool — configuration, never a user.
  An absent count THROWS rather than publishing zero: zero is the healthy side of a
  `GreaterThan`, the same inversion `backup-age.ts` refuses in the other direction.
  The usage budget and the dashboard beside it are console artefacts —
  `docs/reference/DEPLOYMENT.md` is what re-creates them.
