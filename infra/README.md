...
| `template-user.yaml` | The USER stack: one DSQL cluster, the migration Lambda that fills it, and the Cognito pool whose users own its rows. Deployed twice, as `quirenote-backend-user-dev` and `quirenote-backend-user-prod`; the `Environment` parameter has no default, and both the backup tag and the auth domain are derived from it. **Three of the pool's properties cannot be changed after creation** — see [`../docs/reference/COGNITO-POOL-PARAMS.md`](../docs/reference/COGNITO-POOL-PARAMS.md) |
