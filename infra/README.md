...
| `migrations/` | **Reference DDL — nothing reads it.** The applied archive DDL is inline in `ensureSchema`; these files are the pinned contracts and are cited from comments in `capture.ts`. Keeping them in step with `ensureSchema` is manual (A51 found the README claiming otherwise). `001` price_capture · `002` price_observation · **`004` bond_terms (W4)** — `003` is skipped here on purpose, being W7's user-schema draft, which keeps that number on promotion |
