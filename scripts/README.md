...
| `distillation-baseline.ts` | The `pnpm distillation-baseline` CLI, run through `tsx`: rewrites `distillation-baseline.json` (repo root) to the distillation ratchets' current, real counts (repeated sentences, comment volume, history phrases). | Imports `scanRepo` from `../src/distillation/repo-scan` and `BASELINE_PATH`/`serializeBaseline`/`loadBaseline` from `../src/distillation/baseline` — see `src/distillation/README.md`. `src/distillation/distillation-lint.test.ts` is the read-only check this pairs with. |
