...
| `distillation-baseline.ts` | The `pnpm distillation-baseline` CLI, run through `tsx`: rewrites `distillation-baseline.json` (repo root) to the distillation ratchets' current, real counts (repeated sentences, comment volume, history phrases) plus the unrelated docs-line-cap ratchet (§6) that rides the same file. | Imports `scanRepo` from `../src/distillation/repo-scan`, `docLineCounts` from `../src/distillation/doc-line-counts`, and `BASELINE_PATH`/`serializeBaseline`/`loadBaseline` from `../src/distillation/baseline` — see `src/distillation/README.md`. `src/distillation/distillation-lint.test.ts` and `src/docs-line-cap.test.ts` are the read-only checks this pairs with. |
| `build-touch-icon.mjs` | Regenerates `public/apple-touch-icon.png` (the Q-arrow mark, D131) via headless Chrome screenshot — the one copy of the mark no test can guard, since comparing a raster to `public/favicon.svg` needs a renderer the test environment doesn't have. | Run by hand: `node scripts/build-touch-icon.mjs` (needs Chrome; `CHROME` env var overrides the default path). Geometry is duplicated from `favicon.svg` on purpose — see the file's own header comment. `public/README.md` names it in the change-together rule. Unlike the favicon the mark is **inset** here (116 of 180), because iOS masks home-screen icons to a squircle. |
