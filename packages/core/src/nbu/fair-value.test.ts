...
// 2026-08-11. The shape of that file changed four times, so invented fixtures
// would test the parser against the wrong thing.
const HEADER_2026 =
...
describe('parseNbuFairValue', () => {
  ...
  // The whole reason the archive reaches back to 2016: the oldest files carry eight
  // fields and nothing after `maturity`. A parser that requires the tail drops a decade.
  ...
  // The padding fields of the 16-field layout are bare `0`. Taking `cptype` as "the
  // last field" without checking it is not a number reports a type of "0".
  ...
  // The trap this parser exists to avoid: in 2022 the file had SEVENTEEN fields, so
  // `cptype` sits at index 16. Reading a fixed index 17 — the literal reading of the
  // documented index map — returns undefined for every file of that year.
  ...
  // dd.MM.yyyy read as ISO transposes day and month for the first twelve days of
  // every month — a corruption that produces a valid-looking date.
  ...
  // single bad line must not cost the other instruments of that day.
