...
|---|---|---|
| — | Shell + sidebar (all routes) | capital strip, logo, ₴/$ track, nav, group rule, footer band, DEMO badge, version badge, reminder toast |
...
- **Currency track** — first row of the footer band, the only currency indicator in the sidebar. Two segments, **₴** and **$** at 13px, each with its ISO code in an `sr-only` span so the accessible name reads "₴ UAH" and CONTAINS the visible label (2.5.3) — deliberately not an `aria-label`, which would replace it, 22 tall at radius 6 on a track at radius 8 with **no edge**, on the active route's own 12% tint. The selected segment is a solid `accent` chip with `accent-fg` on it, and it still SLIDES (300ms). It spans the band's width; the theme control drawn beside it is #85's, and both are `flex:1`, so its arrival halves this one.
...
Interactions to verify:
1. The Settings pill sits in the sidebar's FOOTER BAND, under the currency track and above the version — not in the scrolling nav, and with no group caption of its own. It activates on `/settings`, where its label reads 3.62 : 1 on the band rather than the 3.88 the other ten read on the wall.
...
**Drawer**
- Tapping the hamburger slides a 280px drawer in from the left over `scrim`. Same navigation as the desktop panel — same `sb-bg` wall, lockup, group rule, pills, footer band with its currency track and Settings, rounded-right corner. It draws no right edge in light (the scrim separates it) and takes the control-boundary rank in dark.
- The Total capital strip is absent in the drawer, and so is the collapse control — the header carries that number.
- The footer band is PINNED: at 740px and at 640px of viewport height its currency track, Settings pill and version badge are all on screen without scrolling the drawer.
