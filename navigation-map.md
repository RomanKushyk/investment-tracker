...
| `/transactions` | Transactions | A32 (2026-08-20); two columns A35 (2026-08-20); **MIRRORED into `/payouts`' grid 2026-08-25 (D88)** | done — the Transaction panel moved here off `/`, and the ledger is the FULL history rather than the last three. The `Entry` group holds this and `/`. **The LEDGER is the wide track and the form the side block on the right** — `grid-cols-[1.6fr_1fr]`, `gap-3.5`, one column below `lg`, measured 676,7 / 14 / 423 at 1440. D88 superseded the drawn arrangement (form 360 on the LEFT, ledger to the remainder): the form is still the narrow column, only the side changed. Below `lg` the form takes the top back with `max-lg:order-first`, because the collapsed DOM order would have put the read-only list above the one control the screen exists for. The ledger's height cap is keyed to `lg` now — as `@min-[944px]` it had no container left and silently sat at the phone's 420 on a desktop. **The ledger's width cap came off (D93, 2026-08-25)** — the card fills its `1.6fr` track at every width; the form keeps its 560 |
...
- **TWO COLUMNS, `/payouts`' GRID SINCE D88 (2026-08-25), MIRRORED.** `where-things-live.dc.html` § S4 drew the form in a 360 column on the LEFT with the ledger taking the rest, and A35 shipped that; D88 superseded the ARRANGEMENT on the owner's reason — the screen read as a different product from the rest of the app. The drawing's real finding stands: the form is the NARROW column and the ledger the wide one, which is the half A32 got wrong. Now `grid-cols-[1.6fr_1fr]`, `items-start`, `gap-3.5`, one column below `lg`, measured 676,7 / 14 / 423 at 1440 — **ledger left, form right**, with the form capped at 560. **The ledger's 884 cap is gone (D93, 2026-08-25, owner ruling):** inside a `1.6fr` track the TRACK is the bound, and a cap narrower than it opened a dead strip between the columns — the ledger now fills its track at every width, and the wide-monitor row stretch is priced in D93. There is no wrap point, no flex basis and no container query left; `transactions-layout.test.ts` pins that none of them come back, and pins the cap's absence too.
- The **Transaction panel**, unchanged in substance — same form, same 9 type options, same atomic `recordTransaction(tx, newAsset)` quick-create path with its dashed "New asset details" sub-card. The form is the NARROW column and never exceeds **560** — beside the ledger its `1fr` track is usually the tighter bound (423 at 1440; the 560 starts binding in-track above a ~1800 viewport), stacked it caps at 560: a form stretched wide reads as a settings page rather than an entry. (An earlier revision of this bullet said 360 beside the ledger — that was the drawn flex basis, gone since D88.)
