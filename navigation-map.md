...
8. **Skip (S5) writes nothing:** press **Skip** → the card collapses, the transaction count is unchanged, `nextCoupon` is unchanged, and `kubushka-settings.state.dismissedReminders` gains `coupon:<assetId>:<date>`. The next coupon date suggests normally; restore every dismissal via Settings→Automation → "Restore dismissed" (checkpoint 19).
