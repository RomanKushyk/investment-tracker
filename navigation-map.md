...
**Drawer**
- Tapping the hamburger slides a 280px drawer in from the left over `scrim`. Same navigation as the desktop rail — same `sb-bg` wall, lockup, pills, currency toggle, rounded-right corner. It draws no right edge in light (the scrim separates it) and takes the control-boundary rank in dark.
