...
1. Type into an empty input → its delta chip computes live vs yesterday's value; pill count increments. Comma and dot decimals both accepted.
2. Typed drafts survive a reload.
3. "Copy yesterday" fills all 4 inputs with yesterday's quotes.
4. "Save snapshot" → toast "Snapshot saved", "Last saved" updates, IndexedDB row is UPSERTED (re-saving the same day does not add a row).
