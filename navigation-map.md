...
- Side panel: **the coupon-due card ALONE, and only on a day one is due** — see the bullet below. The **Transaction** card and the **Recent transactions** card (last 3) both LEFT for `/transactions` on 2026-08-20 (A32), where the list is the full 18-row ledger instead of the last three. *"Interest payout" still renders with a "Coupon" label per the design reference; that check moved with the panel.*
- **THE ASIDE IS CONDITIONAL SINCE A32, AND THE LAYOUT IS NOT.** The Transaction and Recent-transactions cards are gone from this screen. On a day with a coupon due the geometry is unchanged — the S5 card sits in the same `flex-[1_1_300px]` aside. On a day with none the `<aside>` is **not rendered at all** (an empty flex child still claims 300–360px) and the ritual column takes **`max-w-[884px]`** — the app's own `@min-[884px]` number (560 + 24 + 300), not a new one. Measured 2026-08-20 on a no-coupon day: main 1196, ritual column exactly **884**. Without the cap the rows would jump 812 → 1196 the day a coupon is recorded.
