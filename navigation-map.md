...
|---|---|---|
| — | Shell + sidebar (all routes) | capital strip, logo, theme and ₴/$ tracks, nav, group rule, footer band, DEMO badge, version badge, reminder toast |
...
**At and above 768px** (`md`, the app's one breakpoint): **244px** sidebar on `sb-bg` — a step DEEPER than the page in both themes (`#e8e1d6` light, `#0d0d0c` dark), not a dark wall inside a light app — padding 16, right edge rounded **30** — unchanged, though the lockup plate that used to be the inner term of that concentric chain is gone — and a **1px `field-border` stroke on that right edge only** in both themes — visible against the light page, which is #98's one accepted cost. The aside does not scroll with the page; it is FOUR BANDS in three grid rows — the head carries the lockup and, at and above `md`, a bled 56px `sb-field` capital strip; then the scrolling nav; then a pinned footer band on `sb-footer-bg`. The footer band, its currency track, Settings and the version stay on screen at 740px and 640px of viewport height, and the panel only begins to scroll below ~724 (it was ~797 while Settings sat in the nav). On a short window a **12px rail** appears in the nav band, 8px in from its edge, narrowing the nav by 28. Collapsed, the shell is a **56px icon rail** instead: mark, burger, ten 40×36 items with 18px glyphs, one `sb-divider`, and Settings plus a 40×32 currency box in a bled foot band. It carries no capital, no version, no DEMO badge and no theme control — each refused by arithmetic, and each one press away behind Settings or in the header. Below ~596px of viewport height its band scrolls, and there the 12px rail FLOATS at the shell edge instead of reserving a strip — 40px of column has none to give — clearing the centred glyph and passing over the item's outer tint only.

**Below 768px there is no sidebar in flow at all** — see Mobile shell below. The 56px icon rail is a state of the desktop shell and never appears here.

**Floating surfaces** (every route): the `Select` listbox, both `DatePicker` variants, the `Dialog` panel, the toast and the chart tooltip each carry a **1px `field-border` stroke** in both themes — since #98 they read the same control-boundary rank as a field, and in light this is a boundary they previously drew faintly or not at all. The `Dialog` panel and the mobile date sheet float over the overlay instead, and since #92 moved that overlay onto `scrim` they are identified there by their FILL against it in light and by their stroke in dark — so in light their stroke is decorative, which is what #99 now carries.

**Shapes:** nothing in the app is a capsule. Controls take `round(min(w,h) × 0.26)` — badges 5-6, segments 7, small buttons 8, inputs and nav pills 9, larger buttons and segmented boxes 10, plus 4 and 7 (the price-mode toggle's 15px segments round to 4, its track to 4+3=7) and the sidebar's two footer-band tracks 8 with 6px segments. A full-bleed band takes square corners — the sidebar's capital strip and footer band both do, and the band's one met corner is concentric at 29. Only asset avatars and colour dots stay round — the logo mark is drawn geometry, its loop and pills strokes rather than radii. A `rounded-full` capsule anywhere is a regression.

- **Logo lockup** (one row, no plate and no tagline): **22px** 5h mark — a rounded loop with a small second bay, and two pills falling from its right edge, all stroke — beside the wordmark **`quirenote`**, lowercase, JetBrains Mono 600 at −3%, **15px**, in `ink`. Nothing sits behind either part. The mark's three parts are `--color-logo-outline`, `--color-logo-pill-a` and `--color-logo-pill-b`, all three per theme: **`#d8b494` `#ece1d2` `#b07a52`** in dark, **`#8a5a3a` `#b07a52` `#5c3a26`** in light. Nothing in it inherits `currentColor`. Neither part carries the currency symbol or changes on toggle. Same mark in the browser tab (`favicon.svg`, theme-aware, the light trio outside the media query) and as the iOS home-screen icon (180px, opaque `#19181a` plate, the dark trio).
- **DEMO badge:** while demo is active, an amber `DEMO` badge (`warn-tint` inside a 1px `warn` outline — the tint alone does not separate from the wall, radius 5) sits at the END OF THE LOCKUP ROW on every route — in flow, not floating — `title` "Demo dataset — reference data. Switch in Settings → Data."; fades/zooms in on first paint. The lockup row reserves nothing any more — the collapse control left it for the capital strip — so the badge simply ends the row in both shells. The COLLAPSED rail has none — it does not fit the column — and the header carries one beside the figure instead, wherever the header is mounted: always below 768, and while the rail is collapsed above it. So a caution is on screen in every state. With the mobile drawer open both are present, the header's dimmed behind the scrim and `aria-hidden`, so exactly one is announced. Absent in live mode.
- Nav: TWO groups, captions in **11px sentence case** on `sb-label` at `.02em` (not uppercase) with a bare 14px chevron at the row's end — "Ввід/Entry" → "Daily quotes" and "Transactions"; "Аналітика/Analytics" → 8 pills. A **1px `sb-divider` rule**, inset 14, separates them; its 8px margin adds to the nav column's own gap rather than collapsing with it, so the clear space reads **11** at and above `md` and 16 below; it identifies nothing on its own (1.14 : 1) and is a boundary rather than a control. Settings is NOT in this band — it is the footer band's (below). Every item carries a **16px lucide glyph** left of its label, gap 10, `aria-hidden` so the route is not read twice: `/` `CalendarDays`, `/transactions` `ArrowDownUp`, `/overview` `LayoutGrid`, `/balances` `Wallet`, `/payouts` `CircleDollarSign`, `/yield` `ChartLine`, `/attributes` `Tags`, `/seasonality` `CalendarRange`, `/portfolio` `Table`, `/allocation` `ChartPie`, `/settings` `Settings`. Group headers carry none — the chevron there is a control, not a label. A glyph reads `sb-icon` idle and `sb-icon-active` on the current route, and it does NOT follow the label’s hover lift: the record names those two states and no third. With the glyph in the row, «Щоденні котирування» no longer fits its pill and ellipsises — what that should cost is #117. Active pill = `sb-item-active-bg` tint + a **2px inset left indicator** in `sb-indicator` + `aria-current="page"` — never a solid fill; hover is a 5% ground plus a text lift, never a fade. Navigates without full reload.
- **Theme and currency tracks** — one row, the footer band's first, **theme on the left**, each at `flex:1` so it takes half the band's width. One anatomy at two widths: segments **22 tall at radius 6** on a track at radius 8 with **no edge**, on the active route's own 12% tint; the selected segment is a solid `accent` chip with `accent-fg` on it and it SLIDES (300ms); neither track takes `data-filled-track`, because a tint is not the plane's foreground and the base accent ring serves both. **Theme:** three segments, a **14px** lucide glyph each — `Sun` / `Moon` / `Monitor` — idle on `sb-icon` with **no hover** (a glyph has an idle and an active state and no third), 3.33 : 1 in light against 1.4.11's 3; a `radiogroup` of three `radio`s, each announcing the dictionary's word from an `sr-only` span. **Currency:** two segments, **₴** and **$** at 13px, idle on `sb-item` at 4.30 : 1 — 0.20 under 1.4.3 and recorded rather than repaired — each with its ISO code in an `sr-only` span so the accessible name reads "₴ UAH" and CONTAINS the visible label (2.5.3), deliberately not an `aria-label`, which would replace it; a pressed pair rather than radios, because two states of one view are not three exclusive values. Below 768 every one of the five segments is **44 wide** so its `TAP_44` overlay tiles instead of overlapping (3 × 44 + 2 + 4 and 2 × 44 + 1 + 4, plus the drawn 8, is 239 against the **239** the band gives the 280px drawer — the drawer's own 1px right edge is inside its width) and the row wraps to two full-width lines 22 apart rather than clipping if a safe-area inset takes that under 239. **The theme control writes the stored preference and the currency one does not** — see `/settings` below for both halves.
- **Total capital strip** — in the HEAD, under the lockup, at and above `md` only. A bled 56px `sb-field` band with **no radius and no edge**, its caption `sb-label` 9.5px uppercase over `149 016 ₴` (whole ₴) at 18px in `ink` and `+3,08 %` beside it in `pos`, baseline-aligned. The currency counter is dropped. The **collapse control** ends the strip: a bare 24px `ChevronLeft` in `sb-item` inside a 26px pressable box, its ink at x 231, 2px inside the centre of the 20px strip the nav column leaves free — a 26px box centred on 233 cannot fit inside the 244px shell, and the box not needing the shell's clip is what keeps it above 2.5.8's floor. Toggling to $ moves only the thumb and flips the figure; the choice does NOT survive a page reload.
- No sidebar Backup pill — the download lives on `/settings` → Data.
- **Version badge** — last row INSIDE the footer band, centred (`sb-label`, 9.5px uppercase); the collapsed rail carries none, the badge not fitting its column: `v` + the `package.json` version, which it must match exactly. On `sb-footer-bg` it reads 2.87 : 1, the panel's worst text and recorded rather than repaired.
...
- Progress pill **"1 of 4 filled"** (info tint — slate, not the gain green it borrowed before #91) — REIT's quote for 27.07 is already saved. On any later date nothing is saved yet, so the pill reads "0 of 4 filled", every input is empty, and the two bonds show ghost suggestions (below).
...
- Buttons: accent pill **"Save snapshot"**, outline **"Copy yesterday"**; "Last saved 25.07, 21:14" sits at the foot of the rail (or in the row, below the wrap point).
...
1. Type into an empty input → **it groups as you type, in the UI language's own mark** — `1234567` reads `1 234 567` under Ukrainian and `1,234,567` under English, and the decimal you typed is kept (`1 234 567,89`). While you type or edit, the caret stays behind the same DIGIT as the marks move under it; a write the field did not make (the Σ/1 toggle, accepting a ghost) replaces the text and leaves the caret at its end. **What is stored is language-free** — typing `1234` under either language stores `1234` — so switching the language re-draws the same value in the other writing (`1 234` ↔ `1,234`) and every figure derived from it is unchanged. Its delta chip computes live vs yesterday's value; pill count increments. The draft is read under the UI LANGUAGE (D87). In Ukrainian a pasted `4 214,24 грн.` (the issue's NBSP thousands separator is typed as a plain space here; currency suffix with a dot) reads as 4 214,24, and so do `1.234,56` and `1234.56 UAH`; in English the same paste is unreadable — the row shows «Enter a number.» and Save is blocked — while `6,164` reads there as 6 164 and under Ukrainian as 6,164. `₴68,629.36` reads the same in both — two marks present, so the last one is the decimal whatever the language.
...
- Collapsing the desktop sidebar leaves a **56px icon rail** and widens the container by **188**, drawing the 57px header. The choice persists across a reload; below 768 the drawer is the shell and the stored value is not read.
...
- **5 KPI cards**, all currency-aware, mount order Total capital → Capital gain → Total return (net) → Deposited → Free cash:
  - **Total capital 149 016,36 ₴** (the wall tone — `sb-bg` with a 1px `field-border` edge, the sidebar's plane borrowed for one headline card per screen; converts with currency toggle).
...
On seed:
- Area chart of total capital per complete snapshot (Feb→Jul, rising to ~149k): the line, the last-point dot and the hover dot read the accent, the fill its tint — not the gain green, which belongs to deltas. Neither dot carries a ring. The hover cursor is still the `hairline` (#112).
...
On seed:
- Stacked monthly bars (dividends copper, coupons slate, value labels on top).
...
- A three-decimal percent is the case that bites, and the parser follows the language: a stored 6.164 reads back as `6,164` in Ukrainian and `6.164` in English, each accepted by its own UI. The field no longer pads a trailing zero. Pasting `6,164` into the ENGLISH field is refused, but by the 0–100 CAP and not by the grammar: English reads it as a grouped 6164. The same paste into the uncapped "Expected, %" on the same English UI would store 6164, which is why the grammar has to follow the language rather than be caught downstream.
...
- **3 stacked `card` panels** (radius 24) in order: Data → Automation → Appearance, each with a 10px uppercase microlabel.
...
23. **Import — CSV is announced but not yet accepted:** the copy and file picker name `.csv`, but a `.csv` currently gets the unsupported-file-type message.
24. **Two tabs:** open a second tab on `/overview`, import in the first → the second toasts exactly once "Data was replaced in another tab." and re-renders from the new data without a reload; erase/reset broadcast the same way. The acting tab never toasts at itself. Holding the write lock in the other tab makes the confirm sit at "Waiting for another tab…" with both buttons disabled and Esc inert.
...
- Then "TOTAL CAPITAL" in 9.5px uppercase muted over **149 016 ₴** at 18px bold; right, stacked: **+3,08 %** in `pos` over **3 324,03 $** in muted. Toggling the currency in the drawer flips both — same source as the sidebar's capital strip.
- With no KPIs at all the value and delta are both a faint "—".
- Light surface — fills and text are page/ink/muted/pos/neg/hairline only, never the rail's `sb-*`. The focus ring is not in that list: it is the app's ONE base ring, the accent — the sidebar no longer overrides it, and only a filled segmented track repaints it.
- At ≥768 the header is absent while the sidebar is in flow, appearing only when it is collapsed — and there it carries the figure and the DEMO badge but NO burger: the control that expands the rail is in the rail. Below 768 the header keeps its own burger at 18×12, because there is no rail to put one in.

**Drawer**
- Tapping the hamburger slides a 280px drawer in from the left over `scrim`. Same navigation as the desktop panel — same `sb-bg` wall, lockup, group rule, pills, footer band with its theme and currency tracks and Settings, rounded-right corner. It draws no right edge in light (the scrim separates it) and takes the control-boundary rank in dark.
- The Total capital strip is absent in the drawer, and so is the collapse control — the header carries that number.
- The footer band is PINNED: at 740px and at 640px of viewport height its two tracks, Settings pill and version badge are all on screen without scrolling the drawer.
