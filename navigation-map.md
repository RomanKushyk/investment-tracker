...
- **Logo lockup** (one row, no plate and no tagline): **22px** Q-arrow mark — an open Q, gapped top-right, whose tail is a sand arrow — beside the wordmark **`quirenote`**, lowercase, JetBrains Mono 600 at −3%, **15px**, in `ink`. Nothing sits behind either part. The arrow is `--color-brand-sand` and does not change with theme; the arc takes `currentColor` from `ink`, so it follows the wall. Neither part carries the currency symbol or changes on toggle. Same mark in the browser tab (`favicon.svg`, theme-aware) and as the iOS home-screen icon.
