...
- Nav: "DAILY ENTRY" group → "Daily quotes" pill; "ANALYTICS" group → 8 pills; "SETTINGS" group → "Settings" pill. Active pill = `sb-item-active-bg` tint + a **2px inset left indicator** in `sb-indicator` + `aria-current="page"` — never a solid fill; hover is a 5% ground plus a text lift, never a fade. Navigates without full reload.
