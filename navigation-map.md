...
- **Version badge** — last row INSIDE the footer band, centred (`sb-label`, 9.5px uppercase): `v` + the `package.json` version, which it must match exactly. On `sb-footer-bg` it reads 2.87 : 1, the panel's worst text and recorded rather than repaired.
...
**Drawer**
- Tapping the hamburger slides a 280px drawer in from the left over `scrim`. Same navigation as the desktop rail — same `sb-bg` wall, lockup, group rule, pills, footer band with its currency track and Settings, rounded-right corner. It draws no right edge in light (the scrim separates it) and takes the control-boundary rank in dark.
